...
class CallbackResponse(HttpResponse):
    def __init__(self, request: HttpRequest, data: dict, encoder=DjangoJSONEncoder, json_dumps_params=None, **kwargs):
        callback = request.GET.get("callback", "")
        if json_dumps_params is None:
            json_dumps_params = {}
        if callback:
            kwargs.setdefault("content_type", "application/javascript")
            content = f"{callback}({json.dumps(data, cls=encoder, **json_dumps_params)})"
        else:
...
@csrf_exempt
def get_ueditor_settings(request: HttpRequest):
    return CallbackResponse(request, USettings.UEditorUploadSettings)


@csrf_exempt
def get_ueditor_controller(request: HttpRequest):
    """获取ueditor的后端URL地址"""
...
def list_files(request: HttpRequest):
    ...
    if request.method != "GET":
        return CallbackResponse(request, {"state": "ERROR"})
    # 取得动作
    ...
    return CallbackResponse(request, return_info)
...
def UploadFile(request: HttpRequest):
    ...
    # 文件类型检验
    upload_allow_type = {"uploadfile": "fileAllowFiles", "uploadimage": "imageAllowFiles", "uploadvideo": "videoAllowFiles"}
    ...
    # 大小检验
    upload_max_size = {"uploadfile": "filwMaxSize", "uploadimage": "imageMaxSize", "uploadscrawl": "scrawlMaxSize", "uploadvideo": "videoMaxSize"}
...
@csrf_exempt
def catcher_remote_image(request: HttpRequest):
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext in allow_type:
            path_format_var.update({"basename": remote_original_name, "extname": remote_original_ext[1:], "filename": remote_original_name})
            # 计算保存的文件名
...
def get_output_path(request: HttpRequest, path_format, path_format_var):
    # 取得输出文件的路径
...
# 涂鸦功能上传处理
@csrf_exempt
def save_scrawl_file(request: HttpRequest, filename):
    import base64
